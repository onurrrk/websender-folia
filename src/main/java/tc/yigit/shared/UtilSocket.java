...
public static void sendCommand(String command, DataOutputStream out) throws IOException {
        try {
            if (SocketConfig.isBungee()) {
                ...
                if (plugin == null) throw new IllegalStateException("Plugin null!");

                try {
                    Object scheduler = Bukkit.class.getMethod("getGlobalRegionScheduler").invoke(null);
                    scheduler.getClass().getMethod("execute", org.bukkit.plugin.Plugin.class, Runnable.class)
                            .invoke(scheduler, plugin, (Runnable) () -> Bukkit.dispatchCommand(Bukkit.getConsoleSender(), command));
                } catch (Exception e) {
            ...
            UtilSocket.createLog(SocketConfig.prefix + " ERROR: " + ex.getMessage());
            out.writeInt(0);
        }
        out.flush();
    }
